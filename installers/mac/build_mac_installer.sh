...
script_file=SmartAmpPro.pkgproj
...
pkg_dir=SmartAmpPro_Installer_Packaged
rm -Rf $pkg_dir
mkdir $pkg_dir
productsign -s "$TEAM_ID" ../../build/SmartAmpPro.pkg $pkg_dir/SmartAmpPro-signed.pkg

echo "Notarizing installer package..."
INSTALLER_PASS=$(more ~/Developer/mac_installer_pass)
npx notarize-cli --file $pkg_dir/SmartAmpPro-signed.pkg --bundle-id com.GuitarML.SmartAmpPro --asc-provider "$TEAM_ID" --username smartguitarml@gmail.com --password "$INSTALLER_PASS"

echo "Building disk image..."
vol_name=Install_SmartAmpPro-$app_version
