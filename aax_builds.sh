...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    cmake --build build-aax --config $build_config --target SmartAmpPro_AAX | xcpretty

else # Windows
    cmake -Bbuild-aax -G"Visual Studio 16 2019" -A x64
    cmake --build build-aax --config $build_config --parallel $(nproc) --target SmartAmpPro_AAX
fi

# sign with PACE
aax_location=build-aax/SmartAmpPro_artefacts/$build_config/AAX/SmartAmpPro.aaxplugin
wcguid="" # Update
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    wraptool verify --verbose --in $aax_location/Contents/x64/SmartAmpPro.aaxplugin
fi

# reset AAX SDK field...
#sed_cmakelist "s~juce_set_aax_sdk_path.*~# juce_set_aax_sdk_path(NONE)~"

rm -rf "$aax_target_dir/SmartAmpPro.aaxplugin"
cp -R "$aax_location" "$aax_target_dir/SmartAmpPro.aaxplugin"

if [[ "$*" = *deploy* ]]; then
    set +e

    ssh "smartguitarml@gmail.com" "rm -r ~/aax_builds/${TARGET_DIR}/SmartAmpPro.aaxplugin"
